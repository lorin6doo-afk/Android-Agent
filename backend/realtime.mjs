...
const TOOLS = [
  ...
  { type: "function", name: "compose_message", description: "Odpre pogovor z imenikovim stikom s PRIPRAVLJENIM besedilom — v SMS aplikaciji (via:'sms') ali WhatsAppu (via:'whatsapp'). Sporočila NE pošlje — uporabnik pritisne Pošlji sam. Uporabi za osebo, ki je NI v seznamu obvestil.", parameters: { type: "object", properties: { contact: { type: "string", description: "ime stika iz imenika" }, text: { type: "string", description: "pripravljeno besedilo sporočila" }, via: { type: "string", enum: ["sms", "whatsapp"], description: "kanal: 'sms' za SMS aplikacijo, 'whatsapp' za WhatsApp — če ni jasno iz konteksta, uporabnika vprašaj" } }, required: ["contact", "text", "via"] } },
